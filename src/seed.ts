...
const sampleData = [
  {
    ...
    estado: 'ACTIVO',
    ubicacion_pc:
      'C:\\Users\\Clauder Castillo\\Downloads\\MAPEO DE PROCESOS GBS SOLUTIONS\\Mapeo de procesos.docx',
  ...
  },
  {
    ...
    estado: 'ACTIVO',
    ubicacion_pc:
      'C:\\Users\\Clauder Castillo\\Downloads\\MAPEO DE PROCESOS GBS SOLUTIONS\\Junta Clara Dudas Brandon.pdf',
...
if (db) {
  db.serialize(() => {
    ...
    stmt.finalize(() => {
      logger.info('Datos iniciales de prueba insertados correctamente en SQLite.');
      process.exit(0);
    });
  });
} else {
  logger.warn('SQLite no disponible para sembrar datos.');
  process.exit(0);
}
